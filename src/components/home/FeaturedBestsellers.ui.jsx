    ...
    <section className={`${styles.root} w-full py-20 md:py-28 bg-bg-primary overflow-hidden`}>
      <div className="container mx-auto px-6 lg:px-10">
        <div className="flex flex-col items-center justify-center mb-14 text-center">
          <h2 className="font-playfair text-3xl md:text-4xl lg:text-5xl text-text-primary font-medium tracking-wide">{t("Bestsellers.title")}</h2>
              ...
              return (
                <SwiperSlide key={`${product.id}-${index}`}>
                  <div className="flex flex-col group cursor-pointer">
                    <Link href={`/product/${product.id}`} className="relative w-full aspect-[4/5] rounded-2xl overflow-hidden mb-5 bg-[#f4f4f5] dark:bg-white/5">
                      <button onClick={(event) => { event.preventDefault(); event.stopPropagation(); toggleWishlist(product.id); }} className={`absolute top-4 right-4 z-20 w-9 h-9 rounded-full flex items-center justify-center transition-all duration-300 cursor-pointer shadow-sm ${isWishlisted ? "bg-red-500 text-white scale-110" : "bg-white/80 dark:bg-black/40 backdrop-blur-sm text-gray-600 dark:text-gray-300 hover:bg-white dark:hover:bg-black/60 hover:scale-110"}`}>
                        <Heart className={`w-4 h-4 transition-all duration-300 ${isWishlisted ? "fill-white" : ""}`} />
                      </button>
                      ...
                      <div className="absolute bottom-0 left-0 right-0 p-4 translate-y-full group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all duration-400 ease-out z-20">
                        <button onClick={(event) => { event.preventDefault(); event.stopPropagation(); }} className="w-full py-3 rounded-xl bg-white dark:bg-black text-black dark:text-white text-sm font-bold tracking-wide flex items-center justify-center gap-2 shadow-[0_8px_30px_rgba(0,0,0,0.12)] hover:bg-brand-orange hover:text-white transition-colors duration-200 cursor-pointer">

...
const TRUSTED_PAYMENT_REDIRECT_HOSTS = ["paymob.com"];

function isTrustedPaymentRedirect(url) {
  try {
    const { protocol, hostname } = new URL(url);
    if (protocol !== "https:") {
      return false;
    }
    return TRUSTED_PAYMENT_REDIRECT_HOSTS.some(
...
export default function CheckoutPage() {
  ...
  const cartProductIdsKey = cartItems
    .map((item) => `${item.productId}:${item.size}`)
    .sort()
    .join(",");

  // Cart line items can sit in localStorage for weeks with a stale cached
  // price/stock snapshot — re-check against the live catalog as soon as the
  // customer reaches checkout, instead of only finding out an item is gone
  // or out of stock after they've filled out the entire address/phone form
  // and the server rejects the order at the very end.
  useEffect(() => {
    if (!cartProductIdsKey || isBuyNow) {
      return;
    }
    ...
    async function revalidateCart() {
      const uniqueIds = [...new Set(cartItems.map((item) => item.productId))].filter(Boolean);
      ...
      await Promise.all(
        uniqueIds.map(async (id) => {
          try {
            ...
            freshById.set(id, product);
          } catch (error) {
            // Only a confirmed 404 means the product is genuinely gone — a
            // network blip/500 shouldn't wrongly evict a valid cart item,
            // so those are marked "unknown" and left untouched below.
            freshById.set(id, error?.status === 404 ? null : undefined);
          }
        }),
      );
      ...
      const reconciled = cartItems
        .map((item) => {
          const fresh = freshById.get(item.productId);

          // Lookup failed for a reason other than a confirmed 404 (network
          // blip, transient 500) — leave this item exactly as it was rather
          // than guessing.
          if (fresh === undefined) {
            return item;
          }
          ...
          const freshVariant = Array.isArray(fresh.variants)
            ? fresh.variants.find(
                (variant) => String(variant.size).trim().toLowerCase() === String(item.size).trim().toLowerCase(),
              )
            : null;
          ...
          const quantityChanged = clampedQuantity !== item.quantity;
          const stockChanged = freshStock !== item.stock;
          const priceChanged = freshPriceValue !== item.priceValue;

          if (quantityChanged) {
            adjustedTitles.push(item.title);
          }

          // Keep redux in sync with every field that changed — not just
          // quantity — so the mount-sync effect above (which re-fires
          // whenever `reduxCartItems` changes reference) doesn't overwrite
          // this reconciled price/stock right back to the stale cached copy
      ...
      if (hasAnyChange) {
        ...
        if (removedTitles.length > 0) {
          messages.push(
            locale === "ar"
              ? `تم إزالة منتجات لم تعد متاحة: ${removedTitles.join("، ")}`
              : `Removed no-longer-available items: ${removedTitles.join(", ")}`,
          );
        }
        if (adjustedTitles.length > 0) {
          messages.push(
            locale === "ar"
              ? `تم تعديل الكمية حسب المتاح للمنتجات: ${adjustedTitles.join("، ")}`
              : `Adjusted quantity to match available stock for: ${adjustedTitles.join(", ")}`,
          );
        }
  ...
  }, [cartProductIdsKey, isBuyNow, dispatch, locale]);
  ...
  const submitOrder = async (values) => {
    ...
    try {
      ...
      if (nextOrder.paymentMethod === "cash_on_delivery") {
        ...
        router.push(localizePath(`/success?orderId=${nextOrder._id || ""}&paymentMethod=${nextOrder.paymentMethod || "cash_on_delivery"}&finalPrice=${nextOrder.finalPrice || finalTotal}&merchantOrderId=${nextOrder.merchantOrderId || ""}`, locale));
        return;
      }
      ...
      if (
        ...
        if (redirectUrl && isTrustedPaymentRedirect(redirectUrl)) {
      ...
      if (nextOrder.paymentMethod === "kiosk") {
        ...
        router.push(
          localizePath(`/kiosk-success?billReference=${billReference}&finalPrice=${nextOrder.finalPrice || finalTotal}&merchantOrderId=${nextOrder.merchantOrderId || ""}&orderId=${nextOrder._id || ""}`, locale),
        );
      ...
      router.push(
        localizePath(`/success?orderId=${nextOrder._id || ""}&paymentMethod=${nextOrder.paymentMethod || ""}&finalPrice=${nextOrder.finalPrice || finalTotal}&merchantOrderId=${nextOrder.merchantOrderId || ""}`, locale),
      );
    } catch (error) {
      setSubmitError(
        error?.message ||
  ...
  return (
    <div className="w-full bg-bg-primary min-h-screen py-10 md:py-16">
      <div className="container mx-auto px-6 lg:px-10">
        ...
        {cartNotice ? (
          <div className="mb-8 rounded-xl border border-brand-orange/30 bg-brand-orange/10 px-4 py-3 text-sm text-text-primary">
            {cartNotice}
          </div>
        ) : null}
                  ...
                  <div>
                    ...
                    >
                      ...
                      {governoratesList.map((gov) => {
                        ...
                        return (
                          <option key={gov._id || gov.name_en} value={label}>
                            {label} {priceLabel}
                          </option>
                        );
                      })}

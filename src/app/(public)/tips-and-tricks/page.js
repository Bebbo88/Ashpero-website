...
const SmallCard = ({ item, onClick }) => (
  <div
    onClick={onClick}
    className="flex flex-col bg-bg-primary w-full rounded-xl md:rounded-2xl overflow-hidden shadow-sm border border-border-color group cursor-pointer hover:shadow-md transition-all duration-300 flex-1 h-full hover:border-brand-mint/30"
  >
          ...
          {chunkedTips.map((group, rowIdx) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className={`lg:col-span-2 flex ${isReversed ? "lg:order-2" : "lg:order-1"}`}
                >
                ...
                {/* Small Cards Column */}
                <div
                  className={`flex flex-col gap-5 md:gap-6 lg:gap-8 h-full ${isReversed ? "lg:order-1" : "lg:order-2"}`}
                >
                  {smallItems.map((item) => (
                    <div
                      key={item.id}
                      className="flex-1 min-h-[150px] lg:min-h-[200px] flex"
                    >
                      <SmallCard
                        item={item}
              ...
              <div className="p-6 md:p-8 lg:p-12 flex flex-col relative z-20 -mt-6 rounded-t-3xl bg-bg-primary">
                ...
                <div className="font-montserrat text-text-secondary text-sm md:text-base leading-relaxed space-y-6">
                  ...
                  <ul className="list-disc transform space-y-3 ps-5">
                    <li>
                      {isRtl
                        ? "ابدأي بلوحة نظيفة: تأكدي من أن بشرتك خالية تماماً من المكياج والشوائب."
                        : "Start with a clean canvas: ensure your skin is completely free of makeup and impurities."}
                    </li>
                    <li>
                      {isRtl
                        ? "الترتيب هو المفتاح: دائماً طبقي المنتجات من الأخف كثافة إلى الأثقل."
                        : "Layering is key: always apply products from thinnest to thickest consistency."}
                    </li>
                    <li>
                      {isRtl
                        ? "لا تتسرعي: دعي كل طبقة تُمتص بالكامل قبل الانتقال للخطوة التالية."
                        : "Don't rush: allow each layer to fully absorb before moving to the next step."}
                    </li>
                    <li>
                      {isRtl
                        ? "الاستمرارية أهم من الكثافة - العناية اليومية اللطيفة تتفوق على العلاجات القاسية العرضية."
                        : "Consistency matters more than intensity - gentle daily care outperforms harsh occasional treatments."}
                    </li>
                  </ul>

                  <p className="mt-8 text-sm md:text-base italic bg-bg-secondary p-6 rounded-xl border border-border-color">
                    {isRtl
                      ? "تذكري أن تجديد خلايا البشرة يستغرق حوالي 28 يوماً. يجب منح أي روتين جديد شهرًا على الأقل لإظهار فعاليته الحقيقية. التطبيق الصبور والواعي سيثمر دائمًا عن أفضل النتائج لبشرتك."
                      : "Remember that skin cell turnover takes roughly 28 days. Any new routine should be given at least a month to show its true efficacy. Patient, mindful application will always yield the best results for your unique complexion."}
                  </p>
                </div>
              </div>
            </motion.div>
